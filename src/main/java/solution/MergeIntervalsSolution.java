	...
	public int[][] merge(int[][] intervallArray) {
		...
		for (int[] currentInterval : intervallArray) {
			...
			if (mergedIntervals.size() == 0
					|| (currentInterval[0] > mergedIntervals.get(mergedIntervals.size() - 1)[1])) {
			...
			//Case if the two intervals overlap
				...
				int mergedIntervalUpperLimit = mergedIntervals.get(mergedIntervals.size() - 1)[1];
				...
				if (currentIntervalUpperLimit >= mergedIntervalUpperLimit) {
					mergedIntervals.get(mergedIntervals.size() - 1)[1] = currentIntervalUpperLimit;
				}
